  ...
  <div class="post">
    ...
    <ul class="single">
      <li v-for="item in post" :key="item.id">
        ...
        <p class="lim" ref="lim">{{item.lim}}</p>
        <p class="time">{{item.time}}</p>
        <span class="more" @click="morel(item.id)">
        ...
        <ul class="limChange" :key="item.id" v-show="(item.id===nid)&&more">
          <li id="'私密'" ref="open" @click="changeLim('私密',item.id)">私密</li>
          <li id="'公开'" ref="open" @click="changeLim('公开',item.id)">公开</li>
        </ul>
      </li>
    </ul>
  </div>
